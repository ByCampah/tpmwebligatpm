...
export default async function AdminAntiDuPage() {
  ...
  // Agrupar por torneo
  const groupedLobbies: Record<string, typeof lobbies> = {};
  lobbies.forEach(lobby => {
    const key = lobby.tournament?.name || "Otros";
    if (!groupedLobbies[key]) groupedLobbies[key] = [];
    groupedLobbies[key].push(lobby);
  });
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {Object.entries(groupedLobbies).map(([tournamentName, tourneyLobbies]) => (
        <div key={tournamentName} className="flex flex-col gap-4 mt-6">
          <h2 className="text-xl font-bold text-primary border-b border-white/10 pb-2">{tournamentName}</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {tourneyLobbies.map(lobby => (
      ...
      {lobbies.length === 0 && (
        <div className="p-8 text-center text-gray-500 bg-secondary/20 rounded-xl border border-white/5 border-dashed">
          No hay salas Anti-DU creadas. Puedes generarlas desde la pestaña Partidos de un torneo o manualmente arriba.
        </div>
      )}
    </div>
  );
